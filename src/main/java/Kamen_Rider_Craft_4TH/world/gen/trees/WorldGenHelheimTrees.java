package Kamen_Rider_Craft_4TH.world.gen.trees;
...
public class WorldGenHelheimTrees extends WorldGenAbstractTree
{
    ...
    public WorldGenHelheimTrees(boolean p_i2027_1_)
    {
        this(p_i2027_1_, 4, DEFAULT_TRUNK, DEFAULT_LEAF, true);
    }

    public WorldGenHelheimTrees(boolean notify, int minTreeHeightIn, IBlockState woodMeta, IBlockState p_i46446_4_, boolean growVines) {
        super(notify);
                ...
                for (int l = position.getX() - k; l <= position.getX() + k && flag; ++l) {
                    for (int i1 = position.getZ() - k; i1 <= position.getZ() + k && flag; ++i1) {
                        if (j >= 0 && j < worldIn.getHeight()) {
                            if (!this.isReplaceable(worldIn,blockpos$mutableblockpos.setPos(l, j, i1))) flag = false;
                        } else {
                            flag = false;
                        }
                    }
                    ...
                    for (int j3 = 0; j3 < i; ++j3) {
                        ...
                        if (state.getBlock().isAir(state, worldIn, upN) || state.getBlock().isLeaves(state, worldIn, upN) || state.getMaterial() == Material.VINE) {
                            ...
                            if (this.vinesGrow && j3 > 0) {
                                if (rand.nextInt(3) > 0 && worldIn.isAirBlock(position.add(-1, j3, 0))) this.addVine(worldIn, position.add(-1, j3, 0), BlockVine.EAST);

                                if (rand.nextInt(3) > 0 && worldIn.isAirBlock(position.add(1, j3, 0))) this.addVine(worldIn, position.add(1, j3, 0), BlockVine.WEST);

                                if (rand.nextInt(3) > 0 && worldIn.isAirBlock(position.add(0, j3, -1))) this.addVine(worldIn, position.add(0, j3, -1), BlockVine.SOUTH);

                                if (rand.nextInt(3) > 0 && worldIn.isAirBlock(position.add(0, j3, 1))) this.addVine(worldIn, position.add(0, j3, 1), BlockVine.NORTH);
                            }
                        }
                    }
                            ...
                            for (int l4 = position.getX() - k4; l4 <= position.getX() + k4; ++l4) {
                                for (int i5 = position.getZ() - k4; i5 <= position.getZ() + k4; ++i5) {
                                    ...
                                    if (state.getBlock().isLeaves(state, worldIn, blockpos$mutableblockpos1)) {
                                        ...
                                        if (rand.nextInt(4) == 0 && worldIn.isAirBlock(blockpos2)) this.addHangingVine(worldIn, blockpos2, BlockVine.EAST);

                                        if (rand.nextInt(4) == 0 && worldIn.isAirBlock(blockpos3)) this.addHangingVine(worldIn, blockpos3, BlockVine.WEST);

                                        if (rand.nextInt(4) == 0 && worldIn.isAirBlock(blockpos4)) this.addHangingVine(worldIn, blockpos4, BlockVine.SOUTH);

                                        if (rand.nextInt(4) == 0 && worldIn.isAirBlock(blockpos1)) this.addHangingVine(worldIn, blockpos1, BlockVine.NORTH);
                                    }
                                }
                            }
                        }
